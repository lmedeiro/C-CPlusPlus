...
class State: public Peg
	{
		...
		public:
			State();
			State(int n, int start[]);
			State * makeMove();
			State * buildBoard(int,int start []);	// n peg game, start vector, end vector
			void setTime(int);
			short int getColor();
			...
			void setD(int);
			Peg * getAdjList();
			void setPred(State * p);
			short int getPegNum(short int x, short int y);
